...
import type { CreatePostPayload, GetPostsOptions, PostResponse } from '@shared/types';
...
export async function getPosts(nb: number = 5, options?: GetPostsOptions): Promise<PostResponse[]> {
  // Generate API URL
  const url = new URL('/posts', baseApiUrl);
